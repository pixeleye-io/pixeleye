...
func (q *BuildQueries) SquashFailedOrAbortedTargets(ctx context.Context, buildID string) error {
	query := `INSERT INTO build_targets (build_id, target_id) SELECT bt.build_id, bh.parent_id bt.build_id FROM build_targets AS bt JOIN build ON bt.target_id = build.id JOIN build_history AS bh ON bh.child_id = bt.target_id WHERE bt.build_id = $1 AND build.status in ('failed', 'aborted') ON CONFLICT DO NOTHING`
